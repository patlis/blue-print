...
class Patlis_Accommodation_Admin_Settings
{
    ...
    public static function save_settings(): void
    {
        ...
        $confirm_subject = [];
        foreach ((array) ($_POST['confirm_subject'] ?? []) as $lang => $text) {
            $lang = sanitize_key((string) $lang);
            if ($lang !== '') {
                $confirm_subject[$lang] = sanitize_text_field((string) $text);
            }
        }

        $confirm_body = [];
        foreach ((array) ($_POST['confirm_body'] ?? []) as $lang => $text) {
            $lang = sanitize_key((string) $lang);
            if ($lang !== '') {
                $confirm_body[$lang] = wp_kses_post((string) $text);
            }
        }
        ...
        $data = [
            ...
            'booking_email_subject' => isset($_POST['booking_email_subject']) ? sanitize_text_field((string) $_POST['booking_email_subject']) : '',
