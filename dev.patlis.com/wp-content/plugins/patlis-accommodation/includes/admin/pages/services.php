...
const PATLIS_SERVICE_META_HIGHLIGHT = 'patlis_is_highlight';
const PATLIS_SERVICE_META_ORDER     = 'patlis_order';
const PATLIS_SERVICE_META_ICON      = 'patlis_icon';

/**
 * Add form fields (new term)
 */
add_action('property_service_add_form_fields', function () {
  ?>
 ...
 */
add_action('property_service_edit_form_fields', function ($term) {
  $highlight = (int) get_term_meta($term->term_id, PATLIS_SERVICE_META_HIGHLIGHT, true);
  $order     = (int) get_term_meta($term->term_id, PATLIS_SERVICE_META_ORDER, true);
  $icon      = (string) get_term_meta($term->term_id, PATLIS_SERVICE_META_ICON, true);
 ...
 */
$patlis_save_service_meta = function ($term_id) {

  $highlight = !empty($_POST['patlis_is_highlight']) ? 1 : 0;
  update_term_meta($term_id, PATLIS_SERVICE_META_HIGHLIGHT, $highlight);

  $order = isset($_POST['patlis_order']) ? (int) $_POST['patlis_order'] : 0;
  update_term_meta($term_id, PATLIS_SERVICE_META_ORDER, $order);

  $term = get_term($term_id, 'property_service');
  if (!$term || is_wp_error($term)) return;

  $is_parent_term = ((int) $term->parent === 0);

  if (!$is_parent_term) {
    delete_term_meta($term_id, PATLIS_SERVICE_META_ICON);
    return;
  }
  ...
  if ($icon === '') {
    ...
    update_term_meta($term_id, PATLIS_SERVICE_META_ICON, $icon);
  }
};

add_action('created_property_service', $patlis_save_service_meta);
add_action('edited_property_service',  $patlis_save_service_meta);

/**
 * Columns
 */
add_filter('manage_edit-property_service_columns', function ($cols) {
  return [
    'cb'               => $cols['cb'],
    'name'             => __('Name'),
    'patlis_order'     => 'Order',
...
add_filter('manage_property_service_custom_column', function ($content, $column, $term_id) {

  if ($column === 'patlis_order') {
    return (string) (int) get_term_meta($term_id, PATLIS_SERVICE_META_ORDER, true);
  }

  if ($column === 'patlis_icon') {
    $term = get_term($term_id, 'property_service');
    if (!$term || is_wp_error($term) || (int) $term->parent !== 0) return '—';
    $icon = (string) get_term_meta($term_id, PATLIS_SERVICE_META_ICON, true);
    return $icon !== '' ? esc_html($icon) : '—';
  }

  if ($column === 'patlis_highlight') {
    $val = (int) get_term_meta($term_id, PATLIS_SERVICE_META_HIGHLIGHT, true);
    return $val === 1 ? '✓' : '—';
  }
